...
> 最後更新：2026-07-14（v0.0.42.0 T27：15 秒 Host 靜默偵測 + 30 秒總重連窗口 + 重新開頁「返回進行中的房間」+ 後端 membership 綁原 Slot；獨立驗收 7/7，含 Client 原頁面瞬斷後 4.287 秒重連；一次性 local-only hook 已移除）
...
| 斷線重連機制 | ✅ | T24 功能鏈 + T27 v0.0.42.0 實機收口：Host 靜默 15 秒偵測；最後 Host 訊息起 30 秒停止自動重試但保留 Slot；重新開頁可返回 active room、後端綁原 membership slot、Host Full Snapshot 回到 p2。獨立 Browser 驗收已通過 Host 靜默、30 秒窗口、返回原房、p2、畫面同步，以及 Client 原頁面瞬斷後 4.287 秒重連；一次性 local-only hook 已移除。`supabase functions deploy issue-room-join-token --project-ref mezidygnycqtlinoeyml` 於 2026-07-13 重跑成功；T27 正式驗收記錄已追加至 `Docs/history/codex-prompt-T27.md`。 |
...
6. ✅ T27：15/30 秒斷線策略 + 30 秒後重新開頁返回原 Slot；獨立驗收 7/7，Client 原頁面瞬斷後 4.287 秒自動重連，正式驗收記錄已追加
