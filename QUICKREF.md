...
> **當前版本號一律以 `config/gameConfig.js` 的 `GAME_CONFIG.version` 為準**（canonical source）。
> 現行進度：`Docs/planning-dashboard.md`；檔案地圖：`Docs/source-map.md`；任務紀錄：git log + `Docs/history/codex-prompt-T*.md`。
> 本檔的持久化 key / 技術陷阱表仍大致有效，但 v0.0.21.0 之後新增的 key（wallet/shop/skills/inventory/gacha 等，見 `config/economyConfig.js`）不在本檔內。
>
> 版本：v0.0.20.0
