...
> ⚠️ MVP 單機可動：移動/挖礦/背包/塔內資源/跟隨鏡頭/初版建造/核心數值回饋已成循環；波次/戰鬥/晝夜待接。
...
- 目前單機骨架：`main.js` 建立 world、renderer、controls，並用 fixed timestep loop 更新遊戲進程；render 可畫核心/地面/兩層方塊/玩家/建造預覽/核心數值 HUD，input 已接 WASD/方向鍵、長按挖礦、快捷列建造與右鍵拆除。
