...
| `isOnFoundation(world, px, py)` | 判斷座標是否在核心格或 connected dirt 上，供修復/卸貨/站位加成共用 |
...
|---|---|
| `dist2(a,b)` | 平方距離 |
| `selectPrimaryTarget(enemiesInRange, coreCenter)` | 普攻主目標：有敵人正在攻擊核心時優先低血攻擊者，否則鎖最近核心目標 |
...
| `tryDeposit(world, playerId?, cfg?)` | 玩家站在核心或連通泥土地基上 → 背包自動倒入塔內資源欄 |
...
| `updateRepair(world, isRepairing, dt, cfg?, playerId?)` | R 長按修復：站在核心或連通泥土地基上，消耗疲勞回復核心 |
...
| `updateEnemies(world, dt)` | 敵人追最近核心格；進攻擊距離後停下攻擊核心，並標記 `isAttacking` 供核心目標策略使用 |
| `coreAttackAnchors(world)` | 核心本體 + connected dirt 轉成核心攻擊覆蓋 anchor |
| `updateCoreCombat(world, dt, cfg?)` | 核心按 attackSpeed 普攻/連鎖命中範圍內敵人；玩家不在地基上時暫不套用 magicPct；死亡移除 |
...
| 函式 | 職責 |
|---|---|
| `Renderer.render(world)` | 畫地面/網格/礦山方塊/兩層方塊/核心/玩家(插值位置)/敵人小血條/範圍圈/VFX 閃電/建造預覽/核心 HP/疲勞/核心數值 HUD/波次情報；整數像素平移；同步 debug dataset；`firstGame && tutorialTimer > 0` 時疊加教學提示框；`showDebug` 時疊加 debug 浮層 |
...
| `Renderer._drawIntentWheel(world)` | 讀 `world.intentWheel`（由 controls 同步）；Alt+左鍵按下時繪製圓形輪盤（左⛏️上🔧右🧱下🦵中⚠️）；hover 方向高亮；無 wheel 時不繪製 |
| `Renderer._drawEnemyInfo(world)` | 右下可展開波次情報面板；收合顯示當前波敵人，展開後補顯示下一波預覽 |
...
| `TouchControls.attach/detach` | 建立/移除 HTML overlay（D-pad + 動作鍵 + 快捷列 + HTML debug 面板 + 📣意圖選單）；canvas touchstart 監聽 UI hitRects 與卡片選擇 |
| `TouchControls.*`（public 介面） | 與 `Controls` 完全相容：`getMoveVector / isMining / isRepairing / getSelectedSlot / setSelectedSlot / consumePlace / consumeRemove / consumeUiClick / consumeDebugActions / consumeCardChoice / consumeManualIntent` |
...
| `boot()` | 入口：掛版本號；showSplashScreen(diffMode, inputMode, netInfo?) 後：applyTilePx → new Renderer → 選 Controls/TouchControls → touch 才 setupOrientationGuard → resize 監聽 → 建 world → 掛 ⚙ debug 按鈕 → 啟動 fixed timestep loop；本地玩家 auto intent 即時寫入；netInfo 存在時重用 WaitingRoom 的 PeerJS session |
...
| `serializeControls(controls, world, cfg, sequenceId, extra?)` | Client 端：把 controls 狀態打包成 Input Event，含 `manualIntent` |
| `applyInput(world, playerId, input, dt, cfg)` | 單步套用已驗證的 Input 到 world，並維護遠端玩家 auto/manual intent |
...
| 函式 | 職責 |
|---|---|
| `ensureUiState(world)` | 初始化 world.uiState（player/backpack/core/waveInfo expanded 狀態） |
| `applyUiClick(world, uiClick)` | 處理 playerPanel / corePanel / waveInfoPanel 點擊切換 |
