...
> 機器檢查：T27 起的 `codex-prompt-T*.md` 缺任一段，docIntegrity 直接紅燈（`prompt-gmb-missing`）。

### 驗收記錄（審核留痕，T27 起適用）

驗收通過後，**審核方**（不得是實作方本人）在該任務 prompt 檔尾部追加：
...
- 驗收日：YYYY-MM-DD
- 完成標準逐條執行結果：N/N 通過
- 獨立複跑：node tests/index.js → 通過 (vX.Y.Z.W)
- 備註：<發現的問題與處置，無則寫「無」>
```

寫碼方與驗收方必須是不同方（交叉制衡；未來多模型協作時尤其重要）。
機器檢查：驗收記錄一旦存在，四個欄位缺一即紅燈（`acceptance-record-incomplete`）；
「驗收記錄是否存在」由 §12 治理稽核抽查（因 prompt 可先於任務完成前 commit，不能無條件強制）。
...
> 原理：漂移無法防止，只能縮短「漂移到被發現」的時間。機器檢查管狀態規則；
> 本節管機器驗不了的部分（審核品質、語意正確性、規則存活）。

**觸發時機**：每 +10 個功能版本（y 進位滿 10）或開發者指示「治理稽核」時，任一 AI 依下列清單執行：

```
1. dashboard 對帳：隨機抽 3 條 ✅ 條目，實際驗證其證據（跑測試 / 查 commit / 看 config）
2. 驗收記錄抽查：已完成任務的 prompt 是否有驗收記錄；記錄內容與 git log 是否吻合
3. 規則存活檢測：逐條檢查本檔的流程規則（§2 sync-docs、§8 模板、§10 協議），
   問「最近 10 版有被實際執行過的證據嗎」——沒有 = 規則已死，
   依 §10 處理：要嘛恢復執行、要嘛正式修訂規則對齊現實（禁止放著爛）
4. 歸檔巡查：Docs/ 根目錄是否有已完結卻未歸檔的計劃文件；
   凍結文件是否被誤當現況引用
5. 管道健康：§11 管道表逐條實測（gh 可用性、CI 綠、curl 備路可達）
6. 稽核結果記入 dashboard（發現什麼、修了什麼、延後什麼），
   稽核本身也要留下可追溯的記錄
```

上次稽核：2026-07-07（本清單建立日，基準線 = v0.0.41.0 全綠）→ 下次：v0.0.51.0 前後或開發者指示。
