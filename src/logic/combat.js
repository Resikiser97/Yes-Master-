 ...
 * @file        combat.js
 * @module      logic（pure）
 * @summary     核心普攻目標選擇、連鎖目標選擇、單次命中傷害（純函式，隨機注入）
...
// 普攻主目標：混合策略 — 有敵人正在攻擊核心時打血量最低的，否則打距離最近的
export function selectPrimaryTarget(enemiesInRange, coreCenter) {
  const attacking = enemiesInRange.filter(e => e.isAttacking);
  if (attacking.length > 0) {
    let best = null, bestHp = Infinity;
    for (const e of attacking) {
