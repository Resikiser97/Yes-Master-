...
- **現在版本**：v0.0.3.0
- **當前狀態**：MVP 單機可動。移動 / 挖礦 / 背包 / 塔內資源 / 跟隨鏡頭已成循環；建造、波次、戰鬥、晝夜尚未接。
- **下一步（最多 4 條）**：
  1. 步驟 4：建造系統（放置/拆除泥土與第二層方塊，接 `connectivity` 純邏輯）。
  2. 鏡頭死區 + 指數平滑（#3）：實機仍有輕微暈，下次補。
  3. 掉落物系統：背包滿可繼續挖、溢出塊掉地上可撿回。
  4. 步驟 7：波次/晝夜/加時接 game loop（waveGen 已就緒）。
...
### 已知問題
- 🔴 緊急：（無）
- 🟡 待修：建造輸入、波次/戰鬥、晝夜、存檔尚未接成完整可玩循環。
- 🟡 待修：鏡頭實機仍有輕微暈 → 下次加死區 + 指數平滑（#3）。
