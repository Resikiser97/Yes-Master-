...
- **現在版本**：v0.0.4.0
- **當前狀態**：MVP 單機可動。移動 / 挖礦 / 背包 / 塔內資源 / 跟隨鏡頭 / 初版建造 / 核心數值回饋 / 核心 HP 與修復 / 核心戰鬥 / 正式波次晝夜 / 卡片選擇已成完整循環。按 C 可 debug 開抽卡面板。
- **下一步（最多 4 條）**：
  1. 步驟 9：教學流程 / localStorage 存檔（`migration.js` 已就緒）。
  2. 掉落物系統：背包滿可繼續挖、溢出塊掉地上可撿回。
  3. 卡片 UI 手感精調：面板字體 / 色彩 / hover 效果。
  4. 多人接入準備：PeerJS Star 拓撲骨架。
...
### 已知問題
- 🔴 緊急：（無）
- 🟡 待修：正式波次、敵人攻擊核心、晝夜、存檔尚未接成完整可玩循環。
