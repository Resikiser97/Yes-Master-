...
- **現在版本**：v0.0.6.0
- **當前狀態**：MVP 單機可動。移動 / 挖礦 / 背包 / 塔內資源 / **掉落物自動撿取** / 跟隨鏡頭 / 初版建造 / 核心數值回饋 / 核心 HP 與修復 / 核心戰鬥 / 正式波次晝夜 / **卡片選擇（hover + tier 中文化）** / localStorage 存檔 / 新手教學提示已成完整循環。
- **下一步（最多 4 條）**：
  1. 多人接入準備：PeerJS Star 拓撲骨架。
  2. Supabase 帳號系統接入。
  3. 怪物職能擴充（Must Solve 2）。
  4. 建築策略格子化模擬驗證（Must Solve 3）。
