...
- **現在版本**：v0.0.3.0
- **當前狀態**：MVP 單機可動。移動 / 挖礦 / 背包 / 塔內資源 / 跟隨鏡頭 / 初版建造 / 核心數值回饋 / 核心 HP 與修復 / debug 核心戰鬥已成循環；正式波次、晝夜尚未接。
- **下一步（最多 4 條）**：
  1. 步驟 7：正式波次/晝夜/加時接 game loop（waveGen 已就緒）。
  2. 敵人攻擊核心與正式生成位置：把 debug enemy 轉成 waveGen enemy。
