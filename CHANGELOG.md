...
## v0.0.15.0 - 2026-06-27

### 新增
- **手機意圖按鈕**：TouchControls 右側面板新增 📣 手動意圖選單，可送出 ⛏️/🔧/🧱/🦵/⚠️ 或清除意圖，補齊手機版 Alt 輪盤替代操作。
- **波次情報面板**：新增可點擊展開/收合的當前波 / 下一波敵人資訊，顯示敵人數量與基礎 HP。
- **共用地基判斷**：`isOnFoundation()` 移到 `src/logic/connectivity.js`，修復/卸貨/核心魔法站位等共用同一套核心格 + connected dirt 判斷。

### 修復
- **P2 意圖顯示/同步補強**：client input 新增 `manualIntent`，房主端套用遠端玩家手動意圖；本地玩家也在 main loop 即時寫入 auto intent，避免自己或隊友顯示延遲。
- **核心普攻目標選擇**：敵人正在攻擊核心時，核心優先打攻擊中的低血量敵人；沒有攻擊者時才維持最近核心目標。

### 調整
- **核心魔法站位限制**：玩家不在核心或連通地基上時，核心攻擊暫不套用 `magicPct`，讓靈力加成跟站位協作綁定。
- **HUD 密度整理**：縮小背包、核心數值、模式提示與累計資源文字；Party Bar emoji 加底圈提高可讀性；關卡標籤維持設計文件定義的 `0-1` 顯示規則。
- **HUD 對齊**：核心數據 collapsed、XP/金幣/票數文字對齊核心血量 label 視覺中心。
- **意圖輪盤重排**：上⚠️危險 / 下🔧維修 / 左⛏️挖礦 / 右🧱建造 / 中心🦵跑腿；移除死區取消邏輯，不離開中心也可選跑腿。
- **UI 點擊路由**：TouchControls 也會消費 canvas `uiHitRects`，手機可點核心面板 / 波次面板展開收合。
...
## v0.0.14.12 - 2026-06-27

### 新增
- **Alt 意圖輪盤**：Alt + 左鍵 Pressdown 開啟輪盤（圓心 = 按下位置）；拖動方向選擇（左⛏️挖礦 / 上🔧維修 / 右🧱建築 / 下🦵跑腿 / 中⚠️危險）；滑鼠 Pressup 送出意圖（Alt 不需放開）；Alt 放開或移動 < 30px 則取消不送出。
- **角色頭上意圖 Emoji**：所有玩家（本地 + 遠端）頭上顯示 30 秒意圖 Emoji，與 Party Bar 同步。
- **MS6 多人合作分工全部完成**：自動偵測 + 手動輪盤 + 頭上顯示三層全上線。

### 調整
- 輪盤死區判斷：從未移動超過 30px → 取消（不誤送）；曾移出 30px 再拉回中心 → ⚠️。

---

## v0.0.14.11 - 2026-06-27

### 新增
- **B 區隊友列（`_drawPartyBar`）**：多人模式下，頂部水平置中顯示其他玩家卡片（小頭像圓 + 疲勞條）；單人或 players.size ≤ 1 時自動隱藏；離線玩家（`online=false`）頭像半透明 + 疲勞條灰色。
- **多人意圖 Emoji（⛏️🧱🦵🔧）**：本地玩家行為自動寫入 `player.intent / intentAt`（挖礦/建造/拆除/修復）；`stateSync` 同步給隊友；30 秒後自動消失，顯示於隊友卡片頂部。
- **`world.createPlayerState`** 加入 `intent: null` / `intentAt: 0` 初始欄位。

---

## v0.0.14.10 - 2026-06-26

### 新增
- **快捷列 Tooltip**：切換方塊時在快捷列上方顯示 0.5 秒的 tooltip，包含方塊名稱（白色）與每顆對核心的加成效果（金色），最後 0.15 秒淡出。加成名稱對應：核心血量上限、攻擊射程、防禦、攻擊、攻速、穿透、連鎖。`world.hotbarTooltip` 儲存狀態，`_drawHotbarTooltip` 渲染。

---

## v0.0.14.9 - 2026-06-26

### 新增
- **挖礦消耗疲勞**：每成功挖出一塊礦石消耗 5 疲勞（`cfg.player.miningFatigueCost`），疲勞歸零後仍可繼續挖礦（不阻斷）。

### 調整
- **礦山緩衝層**：每座礦山內部從 10×3=30 格改為 10×4=40 格，可見層維持前 3 行（30 塊），第 4 行為隱藏緩衝；補位時從 /40 權重抽取。
- **A 礦山比例（/40）**：`sand:7, stone:7, iron:5, diamond:3, dirt:16, gold:1, glass:1`（泥土比例大幅提升）
- **B 礦山比例（/40）**：`dirt:24, gold:5, glass:4, sand:2, stone:2, iron:2, diamond:1`（泥土進一步集中）
...
### 修復
- **聊天室訊息顯示兩次**：客戶端發送訊息後本機 echo 一次，房主收到後 `broadcast` 又回傳給原始發送者一次，導致看到兩條。修法：`peerHost` 新增 `broadcastExcept(peerId, msg)`，`_onChat` 改呼叫 `broadcastExcept(senderPeer, msg)` 排除原始發送者。
